...
def processjunctionbed(sample,i,jn_list,ex_list):
	...
	jn_dict={}
	ex_dict={}
	sample_junctions=fileread1.readlines()
	all_junctions=fileread2.readlines()
	for line in sample_junctions:
		line.strip()
		data=line.split('\t')
		ii=0
		while ii < len(all_junctions):
...
for x in range(0,int(nloops)):
	...
	for sample in lsamples:
		...
		cmd='bedtools coverage -abam '+bamsample[0]+'.sorted.bam -b '+sample+'.Ex.bed -hist > '+sample+'.Ex.bed2'
		logfile.write(cmd+'\n')
...
localtime = time.strftime("%a:%H:%M:%S")
logfile.write('Finished transforming exon coverage beds to a single co-ordinate system at: ' + localtime + '\n')
